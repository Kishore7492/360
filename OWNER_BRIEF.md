...
*(Updated: 2026-04-19 — SR_FLIP_RETEST stop-loss doctrine corrected to evaluator-owned adaptive structural invalidation)*
...
| `SR_FLIP_RETEST` stop-loss doctrine correction | ✅ In current PR — fixed ±0.20% replaced by adaptive structural invalidation beyond reclaim/wick failure (ATR + structural overshoot buffer) |
...
| Current direction | Run PR-7C observability/validation hardening plus runtime validation of the merged `SR_FLIP_RETEST` structural stop doctrine correction before broader policy moves |
